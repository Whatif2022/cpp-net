...
def get_args_parser():
    ...
    parser.add_argument("--batch_size", default=8, type=int, help="Per GPU batch size") #GPU的块大小
    parser.add_argument("--epochs", default=300, type=int)  #训练300 epoch 改为10 先测试
    parser.add_argument(
    ...
    parser.add_argument("--input_size", default=64, type=int, help="image size") #图像输入大小 从96调整为64
    parser.add_argument("--cs_ratio", default=20, type=int) #压缩感知比从10调整为20
    parser.add_argument(
        "--drop_path",
        type=float,
        default=0.0,
    ...
    # Dataset parameters 数据集的处理方式
    parser.add_argument(
        "--data_path", default=r"D:\deeplearning_python\pythondeepnetProject\CPP-Net-main\data\train", type=str, help="dataset path" #训练数据集路径 绝对地址
    )
    parser.add_argument( #验证数据集路径
        "--output_dir",
        default="./model",
    ...
    parser.add_argument(
        "--eval_data_path",
        default=r"D:\deeplearning_python\pythondeepnetProject\CPP-Net-main\data\val",
        type=str,
...
def main(args):
    utils.init_distributed_mode(args) #初始化分布式训练
    #print(args) #不打印数据参数
    ...
    np.random.seed(seed)
    cudnn.benchmark = True #随机种子保证训练的可重复性

    dataset_train = build_dataset(is_train=True, args=args) #分别构建训练和验证集
