...
class CPP(torch.nn.Module):
    def __init__(self, ratio, block_size=64, dim=8, depth=8, reduction=8): #将大图像切成块32*32,每层卷积生成32个特征图，模型深度，减少通道数 blocksize和dim均从32改为8
        ...
        self.A = nn.Parameter(
            Rearrange("m (1 b1 b2) -> m 1 b1 b2", b1=self.block_size)(A), #问题
            requires_grad=True,
        )
    ...
    def load_sampling_matrix(self):  #压缩感知矩阵
        path = "./data/sampling_matrix"
        #data = np.load(f"{path}/{self.ratio}_{self.block_size}.npy")
        data = np.load (f"{path}/Sensing_Matrix_20_devide8_Normed.npy") #输入我们的矩阵
        return data
...
def load_pretrained_model(ratio, model_name):
    ...
    path = f"./model/checkpoint-{model_name}-{ratio}-best-CYXtest.pth"
    checkpoint = torch.load(path, map_location="cpu")
    return checkpoint      #在这里修改需要保存的模型
...
def cpp8(ratio, pretrained=False, **kwargs):
    model = CPP(ratio, block_size=64, dim=8, depth=8, reduction=8)  #blockSIZE从32调整为8 dim从32改为8
    if pretrained:
        checkpoint = load_pretrained_model(ratio, sys._getframe().f_code.co_name)
        model.load_state_dict(checkpoint["model"])
    return model
